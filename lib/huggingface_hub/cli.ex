defmodule HuggingfaceHub.CLI do
  ...
  def do_process({:user, "login", args}) do
    ...
    IO.puts(
      "Your token has been saved to #{Huggingface_hub.HfFolder.path_token()} and into git credential store"
    )
  ...
  def do_process({:user, "logout", args}) do
    ...
    IO.puts(
      "Your token has been deleted from #{Huggingface_hub.HfFolder.path_token()} and from git credential store"
    )
