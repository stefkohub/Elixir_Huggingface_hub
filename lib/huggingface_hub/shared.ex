defmodule Huggingface_hub.Shared do
  ...
  def currently_setup_credential_helpers(directory \\ ".") do
    {output, errorlvl} =
      System.cmd("git", ["config", "--list"], cd: directory, stderr_to_stdout: true)

    unless errorlvl === 0,
      do: raise(ArgumentError, "Error running git config --list. Details: #{inspect(output)}")
  ...
  def read_from_credential_store(username \\ "") do
    ...
    standard_input =
      (username != "" && standard_input <> "username=#{String.downcase(username)}\n\n") ||
        standard_input <> "\n"
